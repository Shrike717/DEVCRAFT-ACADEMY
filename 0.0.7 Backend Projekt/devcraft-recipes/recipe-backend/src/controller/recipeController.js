...
function checkIfIngredientsIsArrayOfObjectWithRequiredFields(ingredients) {
	console.log(
		'[recipeController.js] checkIfIngredientsIsArrayOfObjectWithRequiredFields: ',
		ingredients
	);
	...
	if (
		!ingredients.every((item) =>
			requiredFields.every((feld) => feld in item)
		)
	) {
		return false;
	}
...
exports.createRecipe = async (req, res) => {
	...
	try {
		const { name, description, steps, cookingTime, userId, ingredients } =
			req.body;
		...
		if (!name || !ingredients || !steps) {
			return res.status(400).json({
				message:
					'Name, steps, und ingredients werden benötigt zum erstellen',
			});
		}
		...
		if (!checkIfIngredientsIsArrayOfObjectWithRequiredFields(ingredients)) {
			return res.status(400).json({
				message:
					'Ingredients muss ein array aus Objekten mit den Feldern: amount,unit und name sein ',
			});
		}

		//Rezept erstellen
		const recipe = await createRecipeModel(
			name,
			steps,
			description,
			cookingTime,
...
exports.updateRecipe = async (req, res) => {
	...
	try {
		...
		if (
			ingredients &&
			!checkIfIngredientsIsArrayOfObjectWithRequiredFields(ingredients)
		) {
			return res.status(400).json({
				message:
					'Ingredients muss ein array aus Objekten mit den Feldern: amount,unit und name sein ',
		...
		const updatedRecipe = await updateRecipeModel(
			id,
			userId,
			name,
			ingredients,
...
exports.getAllRecipes = async (req, res) => {
	try {
		...
		if (allRecipes.recipes.length < 1) {
